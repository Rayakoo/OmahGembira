...
import { getMaterials, updateMaterial, type Material } from "@/services/materials";
...
export default function EditMateri() {
  ...
  useEffect(() => {
    getMaterials(true)
      ...
      .catch(() => router.push("/admin/materi"))
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-8">
        <button onClick={() => router.push("/admin/materi")} className="p-2 rounded-lg hover:bg-foreground/5 transition-colors">
          <ArrowLeft className="w-5 h-5 text-foreground/60" />
        </button>
        <h1 className="text-2xl font-bold text-foreground">Edit Materi</h1>
      </div>
              ...
              {(["video", "pdf"] as const).map((t) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-sm font-medium transition-all uppercase ${
                    type === t ? "bg-charcoal text-white" : "bg-foreground/5 text-foreground/50 hover:bg-foreground/10"
                  }`}
                >
                  {t}
                </button>
            ...
            <div className="rounded-xl overflow-hidden bg-charcoal aspect-video">
              {type === "video" ? (
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <a href={url} target="_blank" rel="noopener noreferrer" className="text-white/60 hover:text-white flex flex-col items-center gap-2 transition-colors">
